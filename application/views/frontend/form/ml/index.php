<!-- /.wrapper -->
<!-- <div class="wrapper image-wrapper bg-image inverse-text" data-image-src="<?php echo base_url(); ?>/assets/images/bg-1.jpeg">
	<div class="container inner pb-140">
		<img src="<?php echo base_url(); ?>/assets/images/uiux.jpg" class="img-fluid rounded mx-auto d-block my-3" alt="uiux.jpg" style="width: 40%; height: 40%;">
		<div class="jumbotron jumbotron-fluid mx-5 ">
			<div class="container">
				<h1 class="display-5 text-center">Pendaftaran Lomba UI/UX VOC 2021</h1>
				<div class="rounded-lg m-3">
					<p>
						Biaya Pendaftaran Rp. 80.000,- per team. <br>
						Biaya Pendaftaran dapat ditransfer ke rekening BNI (1134128079 - Rizquna), BCA (2380914215 - Rizquna), Mandiri (1850000496072 - Exca). <br> <br>
						Setelah peserta melakukan pendaftaran dan pengumpulan karya di google form yang telah disediakan maka peserta wajib mengirim pesan konfirmasi kepada Mafa (0895377205773) dihari yang sama. Disertai dengan bukti pembayaran.
					</p>
					<hr>
					<form id="form">
						<div class="form-group">
							<label>Nama Ketua Tim</label>
							<input type="ketua" class="form-control" id="namaketua">
							<div class="msg-err-namaketua"></div>
						</div>
						<div class="form-group">
							<label>Email Ketua Tim</label>
							<input type="ketua" class="form-control" id="emailketua">
							<div class="msg-err-emailketua"></div>
						</div>
						<div class="form-group">
							<label>Nomor Hp Ketua Tim</label>
							<input type="ketua" class="form-control" id="nomorketua">
							<div class="msg-err-nomorketua"></div>
						</div>
						<div class="form-group">
							<label>Nama Anggota</label>
							<input type="ketua" class="form-control" placeholder="Anggota 1" id="anggota1">
							<div class="msg-err-anggota1"></div>
						</div>
						<div class="form-group">
							<input type="ketua" class="form-control" placeholder="Anggota 2" id="anggota2">
							<div class="msg-err-anggota2"></div>
						</div>
						<div class="form-group">
							<label>Asal Instansi</label>
							<input type="ketua" class="form-control " id="instansi">
							<div class="msg-err-instansi"></div>
						</div>
						<div class="text-center">
							<button type="submit" class="btn btn-primary btn-lg" id="submit-btn" disabled>Submit</button>
						</div>
					</form>
				</div>
			</div>
		</div>
	</div>
</div> -->
<div class="wrapper white-wrapper">
	<img src="<?= base_url(); ?>/assets/images/pubg1.png" class="img-fluid rounded mx-auto d-block my-3" alt="pubg.png" style="width: 70%; padding-top: 50px;">
	<div class="jumbotron jumbotron-fluid mx-5">
		<div class="container">
			<h1 class="display-4 text-center">Pendaftaran Lomba Mobile Legend VOC 2021</h1>
			<div class="bg-light rounded-lg" style="padding: 20px;">
				<!-- <p>
					Biaya Pendaftaran Rp. 80.000,- per team. <br>
					Biaya Pendaftaran dapat ditransfer ke rekening BNI (1134128079 - Rizquna), BCA (2380914215 - Rizquna), Mandiri (1850000496072 - Exca). <br> <br>
					Setelah peserta melakukan pendaftaran dan pengumpulan karya di google form yang telah disediakan maka peserta wajib mengirim pesan konfirmasi kepada Mafa (0895377205773) dihari yang sama. Disertai dengan bukti pembayaran.
				</p>
				<hr> -->
				<form id="form">
					<div class="form-group">
						<label>Nama Team</label>
						<input type="ketua" class="form-control" id="namatim">
						<div class="msg-err-namatim"></div>
					</div>
					<div class="form-group">
						<label>Email</label>
						<input type="ketua" class="form-control" id="email">
						<div class="msg-err-emailketua"></div>
					</div>
					<div class="form-group">
						<label>Asal Instansi</label>
						<input type="ketua" class="form-control" id="instansi">
						<div class="msg-err-instansi"></div>
					</div>
					<div class="form-group">
						<label>Nama Kapten</label>
						<input type="ketua" class="form-control" id="namaketua">
						<div class="msg-err-namaketua"></div>
					</div>
					<div class="form-group">
						<label>No Hp Kapten</label>
						<input type="ketua" class="form-control" id="nomorketua">
						<div class="msg-err-nomorketua"></div>
					</div>
					<div class="form-group">
						<label>ID Kapten</label>
						<input type="ketua" class="form-control" id="idketua">
						<div class="msg-err-idketua"></div>
					</div>
					<div class="form-group">
						<label>Scan KTM/KTP/NISN</label>
						<input type="file" class="form-control " id="scan">
						<div class="msg-err-scan"></div>
					</div>
					<div class="form-group">
						<label>Bukti Transfer</label>
						<input type="file" class="form-control " id="tf">
						<div class="msg-err-tf"></div>
					</div>
					<div class="form-group">
						<label>Detail Kelengkapan</label>
						<input type="file" class="form-control " id="kelengkapan">
						<div class="msg-err-kelengkapan"></div>
					</div>
					<div class="text-center">
						<button type="submit" class="btn btn-primary btn-lg" id="submit-btn" disabled>Submit</button>
					</div>
				</form>
			</div>
		</div>
	</div>
	<!-- /.container -->
</div>
